...
def check_for_macgpg2
  if File.exist? "/Applications/start-gpg-agent.app" or
    ...
    puts <<-EOS.undent
      ...
      /usr/homebrew and permissions issues with share and man in /usr/homebrew/.
...
def check_for_stray_dylibs
  unbrewed_dylibs = Dir['/usr/homebrew/lib/*.dylib'].select { |f| File.file? f and not File.symlink? f }
  ...
  opoo "Unbrewed dylibs were found in /usr/homebrew/lib"
  puts <<-EOS.undent
    You have unbrewed dylibs in /usr/homebrew/lib. If you didn't put them there on purpose,
...
def check_homebrew_prefix
  unless HOMEBREW_PREFIX.to_s == '/usr/homebrew'
    puts <<-EOS.undent
      You can install Homebrew anywhere you want, but some brews may only work
      correctly if you install to /usr/homebrew.
...
def check_for_multiple_volumes
  ...
  tmp_prefix = ENV['HOMEBREW_TEMP'] || '/usr/homebrew/tmp'
